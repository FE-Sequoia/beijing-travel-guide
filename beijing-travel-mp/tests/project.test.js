...
const foodJs = fs.readFileSync(path.join(root, 'pages/food/index.js'), 'utf8');
const foodDetailWxml = fs.readFileSync(path.join(root, 'pages/food-detail/index.wxml'), 'utf8');
const foodDetailJs = fs.readFileSync(path.join(root, 'pages/food-detail/index.js'), 'utf8');
assert.match(foodWxml, /class="food-grid"/, '美食页应使用双列卡片容器');
assert.match(foodWxss, /\.food-grid\s*\{[^}]*grid-template-columns:\s*repeat\(2,\s*minmax\(0,\s*1fr\)\)/s, '美食页应采用两列网格');
assert.match(foodWxml, /bindtap="onFood"/, '美食卡片应可进入详情');
assert.match(foodJs, /pages\/food-detail\/index\?id=/, '美食列表应导航至详情页');
assert.match(foodWxml, /wx:for-item="food"/, '美食卡片循环应为餐厅记录命名，避免与星级循环变量冲突');
assert.match(foodWxml, /wx:for-item="star"/, '星级循环应使用独立变量');
...
assert.match(placeDetailWxss, /\.copy\s*\{[^}]*line-height:\s*1\.6/s, '景点正文应使用紧凑行高');
assert.match(guideDetailWxss, /\.copy\s*\{[^}]*line-height:\s*1\.6/s, '攻略正文应使用紧凑行高');
