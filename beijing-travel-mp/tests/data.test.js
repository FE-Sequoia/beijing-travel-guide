...
assert.ok(searchAll('长城').itineraries.every((item) => Array.isArray(item.placeNames)), '行程搜索结果应附带途经景点名');
...
assert.ok(foods.every((food) => !placeIds.has(food.id)), '美食 ID 不应与景点 ID 冲突');
foods.forEach((food) => {
